...
namespace Akka.Persistence.Sql.Common.TestKit
{
    public abstract class SqlJournalQuerySpec : PluginSpec
    {
        ...
        {
            return string.Format(@"{0}.timestamp-provider = ""{1}, {2}""", plugin, typeof(TestTimestampProvider).FullName, typeof(TestTimestampProvider).Assembly.GetName().Name);
        }
        ...
        private void QueryAndExpectSuccess(Query query, params IPersistentRepresentation[] events)
        {
            ...
            {
                ExpectMsg<QueryResponse>(q => q.QueryId == query.QueryId &&
